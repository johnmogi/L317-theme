...
add_filter('woocommerce_checkout_customer', function($customer_data) {
    if (!is_user_logged_in() && !empty($customer_data['user_login'])) {
        ...
        // Use ID number as password if available, otherwise generate random password
        if (!empty($customer_data['id_number'])) {
            $customer_data['user_pass'] = sanitize_text_field($customer_data['id_number']);
        } else {
